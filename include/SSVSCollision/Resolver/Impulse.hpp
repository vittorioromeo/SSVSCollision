...
public:
    ...
    {
        const auto& newStress(nextStress + ssvs::getAbs(getBody().getInvMass() *
                                                        mStress * stressMult));
...
struct Impulse
{
        ...
        for(const auto& b : mToResolve) {
               ...
               (oldShapeLeftOfS || (os.isRightOf(shape) && oldVOverlap)))
            {
                ...
                desiredX *= mBody.getRestitutionX();
                    ...
                    (oldShapeRightOfS || (os.isLeftOf(shape) && oldVOverlap)))
            {
            ...
            if(normal.y != 0) {
                ...
                if(b->velTransferMult.x != 0)
                    velTransferX *=
                    std::sqrt(mBody.velTransferMult.x * b->velTransferMult.x);
                else
                    velTransferX *= 0;
            ...
            if(normal.x != 0) {
                ...
                if(b->velTransferMult.y != 0)
                    velTransferY *=
                    std::sqrt(mBody.velTransferMult.y * b->velTransferMult.y);
                else
                    velTransferY *= 0;
    ...
    inline void postUpdate(TW& mWorld) const
    {
        for(const auto& b : mWorld.getBodies()) {
            ...
            ssvu::NumLimits<float>::min(), ssvu::NumLimits<float>::max());
