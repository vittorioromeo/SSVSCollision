...
struct BodyData
{
    ...
    inline BodyData(bool mIsStatic, const Vec2i& mPos,
    const Vec2i& mSize) noexcept : shape{mPos, mSize / 2},
                                   oldShape{shape},
                                   _static{mIsStatic}
    {
    }

    inline void setMass(float mX) noexcept
    {
        mass = mX;
        invMass = (mX == 0 ? 0 : (1.f / mX));
    }
};
}
