...
namespace GridQueryTypes
{
    ...
    namespace Bodies
    {
        ...
        struct All
        {
            ...
            {
                mBodies = mInternal.grid.getCell(mInternal.index).getBodies();
            }
        };
        template <typename TW>
        struct ByGroup
        {
            template <typename T>
            inline static void getBodies(
            std::vector<Body<TW>*>& mBodies, const T& mInternal, Group mGroup)
            {
    ...
    struct RayCast : public Base<TW, TGrid>
    {
        ...
        RayCast(TGrid& mGrid, const Vec2i& mPos, const Vec2f& mDir)
            : Base<TW, TGrid>{mGrid, mPos}, cellSize{this->grid.getCellSize()},
              dir{mDir}, increment{dir * ssvu::toFloat(cellSize)},
        ...
        inline bool getSorting(const Body<TW>* mA, const Body<TW>* mB)
        {
            return ssvs::getDistEuclidean(mA->getPosition(), this->startPos) >
                   ssvs::getDistEuclidean(mB->getPosition(), this->startPos);
        }
        inline bool hits(const AABB& mShape)
        {
    ...
    struct Distance : public Base<TW, TGrid>
    {
        ...
        Distance(TGrid& mGrid, const Vec2i& mPos, int mDistance)
            : Base<TW, TGrid>{mGrid, mPos}, cellSize{this->grid.getCellSize()},
              distance{mDistance}, cellRadius{distance / cellSize}
        {
