...
namespace ssvsc
{
	template<typename T> struct Segment
	{
		...
		inline Segment(const Vec2<T>& mStart, const Vec2<T>& mEnd) noexcept : start{mStart}, end{mEnd} { }

		inline auto getPointCrossProduct(const Vec2<T>& mPoint) const noexcept	{ return ((end.x - start.x) * (mPoint.y - start.y) - (end.y - start.y) * (mPoint.x - start.x)); }
